    ...
    for (int x = min_x; x <= max_x; x++)
    {
        for (int y = min_y; y <= max_y; y++)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 2; j++)
                {
                    if (insideTriangle(x + 0.5 * i + 0.25, y + 0.5 * j + 0.25, t.v))
                    {
                            auto[alpha, beta, gamma] = computeBarycentric2D(x + 0.5 * i + 0.25, y + 0.5 * j + 0.25, t.v);
                            float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                            float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                            z_interpolated *= w_reciprocal;
                            int idx = super_get_index(x * 2 + i, y * 2 + j);
                            Vector3f point(x, y, z_interpolated);
                            if (z_interpolated < super_depth_buf[idx])
                            {
                                // TODO : set the current pixel (use the set_pixel function) to the color of the triangle (use getColor function) if it should be painted.
                                ...
                                for (int ii = 0; ii < 2; ii++)
                                {
                                    ...
                                    {
                                        color[0] += 0.25 * super_frame_buf[super_get_index(x * 2 + ii, y * 2 + jj)][0];
                                        color[1] += 0.25 * super_frame_buf[super_get_index(x * 2 + ii, y * 2 + jj)][1];
                                        color[2] += 0.25 * super_frame_buf[super_get_index(x * 2 + ii, y * 2 + jj)][2];
                                    }
                                }
